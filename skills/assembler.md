...
## 3. Dynamic Sequential Scheduling Gate (Max 3 Posts/Day, 8-Hour Intervals)
...
- **Find the next available slot**: The allowed daily slots are strictly:
  ...
  - 08:00:00 +0900
  - 16:00:00 +0900
- **Sequence Assignment**: Assign the new post to the first empty slot. If all 3 slots for a day are occupied, automatically shift the new post to the `00:00:00` slot of the next calendar day.
...
- **Check 3 (Conversational Q&A)**: Ensure the post strictly employs the Host-and-Expert interview layout (using `### Q.` headers and `**A.**` tags) to break down the core scientific concepts.
...
- **Check 7 (Highlighting)**: Ensure key sentences in the answer blocks are highlighted using the `<mark style="background: rgb(221, 235, 241);">**bold highlighted text**</mark>` syntax.
